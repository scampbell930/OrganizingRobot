...
def move_robot(clientID, coordinates):
 ...
 for coord in coordinates:
    ...
    err_code = vrep.simxSetObjectPosition(clientID, dummy_handle, -1 , [coord.x,coord.y,0.2], vrep.simx_opmode_blocking)
    ...
    while True:
        ...
        errcode, robot_orientation = vrep.simxGetObjectOrientation(clientID,robot_handle,dummy_handle,vrep.simx_opmode_blocking)
        


        errcode, robot_position = vrep.simxGetObjectPosition(clientID,robot_handle,-1,vrep.simx_opmode_blocking)
        errcode, vision_position = vrep.simxGetObjectPosition(clientID,vision_handle,-1,vrep.simx_opmode_blocking)
        current_vector_from_points =  [(vision_position[0] - robot_position[0]) , (vision_position[1] - robot_position[1])]
    ...
    while True:
        ...
        if(round(coord.x,1) - 0.15 <= round(robot_position[0],1) <= round(coord.x,1) + 0.15 and  round(coord.y,1) - 0.15 <= round(robot_position[1],1) <= round(coord.y,1) + 0.15):
